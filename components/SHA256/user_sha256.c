...
#include <stdint.h>
#include <string.h> // CBC_T mode, for memset
#include "user_sha256.h"
...
void hmac_sha256(const unsigned char *data, int len, const unsigned char *key, int len_key, unsigned char *out) {
    int block_size = SHA256_BLOCK_SIZE;
    int hash_size = SHA256_BYTES;
    ...
    // int hash_buf_size = key_size + (len < hash_size ? hash_size : len);
    unsigned char hash_buf[SHA256_BLOCK_SIZE*2];
    memcpy(hash_buf, buf2, key_size);
    memcpy(hash_buf + key_size, data, len);
    unsigned char hash_out[SHA256_BYTES];
